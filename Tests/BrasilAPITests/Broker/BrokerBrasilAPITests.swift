...
final class BrokerBrasilAPITests: XCTestCase {
    ...
    func test_listBrokers() async throws {
        ...
        let brokers: [BrasilAPI.Broker.BrokerModel] = try await brasilAPI.broker.getBrokers()
    ...
    func test_getBrokersByCNPJ() async throws {
        let brasilAPI = BrasilAPI(brokerRepository: repository)
        let broker: BrasilAPI.Broker.BrokerModel = try await brasilAPI.broker.getBrokerByCnpj(cnpj: "76621457000185")
