...
final class BankBrasilAPITests: XCTestCase {
    ...
    func test_listBanks() async throws {
        ...
        let banks: [BankingService.BrasilAPIBankModel] = try await brasilAPI.banking.listBanks()
    ...
    func test_getBankById() async throws {
        let brasilAPI = BrasilAPI(bankingRepository: repository)
        let bank: BankingService.BrasilAPIBankModel = try await brasilAPI.banking.getBankByCode(code: "001")
